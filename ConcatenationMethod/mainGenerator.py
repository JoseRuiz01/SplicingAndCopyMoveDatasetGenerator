...
original_dataset_path, splicing_dataset_path, copy_move_dataset_path = createDatasetFolders()
...
for original_audio_path in timit_files:
    ...
    # Get rid of the path to TIMIT folder and the .WAV
    audio_path_normalized, _ = os.path.splitext(os.path.basename(original_audio_path))

    # Save audios with forgery (length 3s) to folder
    for idx, audio in enumerate(copyMove_audios):
        copy_move_audio_path = getPath(f"{audio_path_normalized}_{idx}.WAV", copy_move_dataset_path)
        soundfile.write(copy_move_audio_path, audio, sr)

    for idx, audio in enumerate(splicing_audios):
        splicing_audio_path = getPath(f"{audio_path_normalized}_{idx}.WAV", splicing_dataset_path)
